...
{
    public class ClientLoadingLevel : BaseState
    {
        ...
        {
            Debug.Log("ClientLoadingLevel.Initialize: level=" + levelIdx.ToString() + "\n");
            bLoadedLevel = false;
        ...
        //{
        //    Debug.LogWarning("clientLoadingLevel.Start: level=" + levelIdx.ToString() + "\n");
        //}

        // Update is called once per frame
        public override void Update()
        {
            //Debug.LogWarning("clientLoadingLevel.Update: bLoadedLevel=" + bLoadedLevel.ToString() + "\n");
            if (immediateLevelLoad) //  otherwise, we'll go through the menu.
            {
                ...
                //Debug.Log("clientLoadingLevel.Update: level=" + levelIdx.ToString() + "\n");
                //Debug.Log("clientLoadingLevel:curState=" + curState + "(" + GetState(curStateName).ToString() + ")\n");
                ...
                if (GetState(curStateName) == originalMHStates.ChooseSignInMenu)    //  skip the sign in menu since we're the client.
                {
                    bLoadedLevel = GentleLoadLevel(levelIdx);    //  force the MainGame state to load the level that was requested. 
                }
            }
            //  weird... we can load the level without this variable being set somehow. Okay, just check to see if we're in the game play state and then change our client state!
            //if (bLoadedLevel)   //  if the level is loaded, we can see if we're in a state to start the Pre-Openmatch gameplay.
            {
                ...
                {
                    //  the client has finished loading the map and is ready to let players drop in. So go to the next state.
                    //  Allow players to drop in to the game now.
                    if (MultiplayerGame.instance.agones != null)
                    {
                        //MultiplayerGame.instance.ClientEnterMultiPlayerState<Hamster.States.clientListenForClients>();
                    }
                    else
                    {
                        //MultiplayerGame.instance.ClientEnterMultiPlayerState<Hamster.States.clientPreOpenMatchGamePlay>();
                    }
                }
            }
        }
